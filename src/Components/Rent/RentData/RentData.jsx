...
function RentData({ button = false, from }) {
  ...
  return (
    ...
    >
      {/* Location Box */}
      <div
        className={`flex flex-col gap-1.5 md:gap-3 md:border-none border-1 border-[#dadada] rounded-xl p-2.5 cursor-pointer ${
          from === true
            ? "col-span-1"
      ...
      {/* Pickup Box */}
      <div
        className={`flex flex-col gap-3 md:justify-center items-start md:items-center md:border-none border-1 border-[#dadada] rounded-xl p-2.5 cursor-pointer  ${
          from === true ? "col-span-1" : "col-span-2 md:col-span-1"
        }`}
        onClick={() => enablePickUp(true)}
      >
        <div className="flex flex-col gap-3 md:justify-center items-start ">
          <span className="text-N5 text-xs md:text-sm font-medium text-nowrap">
            Pickup date and time
          </span>
          <div className="flex gap-2 md:gap-3 items-start md:items-center  w-full">
            <img src={Calendar} />
      ...
      {/* Drop Box */}
      <div
        className={` cursor-pointer md:border-none border-1 border-[#dadada] rounded-xl p-2.5  ${
          from === true
            ? "col-span-1"
